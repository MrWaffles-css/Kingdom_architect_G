-- FIX: Spy Player Error
-- Overwrites the spy_player function to ensure no invalid field access
-- Combines Vault Steal Logic + Spy Report Storage
...
BEGIN
    ...
    -- Get target's sentry and stats (from user_stats, which has NO username)
    ...
    IF my_spy > their_sentry THEN
        ...
        -- Save spy report to database (if table exists)
        -- We construct the query carefully to avoid errors if table missing
        BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
            -- Ignore errors if spy_reports table missing or other issues
            -- This ensures the basic spy function still returns data
            NULL;
        END;
