-- Add new configuration variables for attack mechanics
INSERT INTO public.game_config_variables (key, value, description)
VALUES 
    ('attack_turn_cost', 100, 'Number of turns required to launch an attack.'),
    ('attack_experience_gain', 100, 'Experience gained by the attacker upon a successful attack.')
ON CONFLICT (key) DO NOTHING;

-- Update attack_player function to use these dynamic configurations
...
BEGIN
    ...
    v_turn_cost := public.get_game_config_variable('attack_turn_cost', 100);
    v_experience_gain := public.get_game_config_variable('attack_experience_gain', 100);
    ...
    IF v_attacker_stats.turns < v_turn_cost THEN
        RAISE EXCEPTION 'Not enough turns (Need %)', v_turn_cost;
    END IF;
