-- Helper function to force update resources for ANY user (used by Spy/Attack)
-- This ensures offline players have up-to-date resources before being interacted with.

CREATE OR REPLACE FUNCTION public.force_update_resources(p_target_id uuid)
RETURNS void
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
    ...
    -- Get current stats of the target
    SELECT * INTO v_stats FROM public.user_stats WHERE id = p_target_id;
    
    IF v_stats IS NULL THEN
        RAISE NOTICE 'Target stats not found for force update';
        RETURN;
    END IF;
    ...
    -- If less than 1 minute has passed, do nothing
    ...
    IF v_vault_level > 0 THEN
        ...
        -- Updated Vault Capacities (matching fix_turn_generation.sql logic roughly or generally understood values)
        -- Using explicit values for safety
