-- Fix Gold Overflow Logic in Battle
-- Ensures that if gold stolen exceeds vault capacity, the overflow goes to the user's main gold balance.
...
DECLARE
    ...
    v_vault_cap bigint;
    v_vault_space bigint; -- New variable
    v_overflow bigint;    -- New variable
    v_kill_rate float;
    v_loss_rate float;
    v_report_id uuid; 
...
BEGIN
    ...
    IF v_attacker_stats.attack > v_defender_stats.defense THEN
        ...
        -- === NEW OVERFLOW LOGIC ===
        -- Calculate available space in vault
        v_vault_space := GREATEST(0, v_vault_cap - COALESCE(v_attacker_stats.vault, 0));

        IF v_gold_stolen <= v_vault_space THEN
            -- Fits entirely in vault
            UPDATE public.user_stats
            SET vault = vault + v_gold_stolen,
                experience = experience + 100
            WHERE id = v_attacker_id;
        ELSE
            -- Fills vault, overflows to gold
            v_overflow := v_gold_stolen - v_vault_space;
            UPDATE public.user_stats
            SET vault = v_vault_cap, -- Filled to maximize capacity
                gold = gold + v_overflow, -- Overflow goes to on-hand gold
                experience = experience + 100
            WHERE id = v_attacker_id;
        END IF;
