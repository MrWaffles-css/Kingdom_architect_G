-- Allow Vault Overflow from Looting
-- Run this in Supabase SQL Editor

-- Update attack_player function to allow vault overflow from looting
DROP FUNCTION IF EXISTS attack_player(UUID);
...
BEGIN
    ...
    IF v_attacker_stats.attack > v_defender_stats.defense THEN
        ...
        -- Probabilistic rounding: floor + (1 if random < decimal_part else 0)
        v_casualty_count := floor(v_raw_casualties) + (CASE WHEN random() < (v_raw_casualties - floor(v_raw_casualties)) THEN 1 ELSE 0 END);
        v_stat_loss := v_casualty_count * 100;

        -- CHANGED: Allow vault overflow - no capacity check
        -- Gold from looting can exceed vault capacity
        UPDATE public.user_stats
        SET vault = vault + v_gold_stolen,
            experience = experience + 100
        WHERE id = v_attacker_id;

        -- Update Defender (Lose Gold, Soldiers, Defense)
        UPDATE public.user_stats
        SET gold = 0,
            defense_soldiers = GREATEST(0, defense_soldiers - v_casualty_count),
            defense = GREATEST(0, defense - v_stat_loss)
        WHERE id = target_id;
    ...
    ELSE
        ...
        -- Update Attacker (Lose Soldiers, Attack)
        UPDATE public.user_stats
        SET attack_soldiers = GREATEST(0, attack_soldiers - v_casualty_count),
            attack = GREATEST(0, attack - v_stat_loss)
        WHERE id = v_attacker_id;
...
-- Done! Looted gold can now exceed vault capacity.
-- Next: Update App.jsx to block passive gold when vault is over capacity.
