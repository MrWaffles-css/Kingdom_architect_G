...
-- This prevents race conditions and ensures immediate responsiveness without client-side lag.
...
BEGIN
    ...
    -- Calculate Cost (Base Cost * 10^Tier)
    -- Tier 0: 1,000
    -- Tier 1: 10,000
    -- Tier 2: 100,000
    -- Tier 3: 1,000,000
    -- Tier 4: 10,000,000
    -- Tier 5: 100,000,000
    v_unit_cost := 1000 * (10 ^ p_tier);
    v_total_cost := v_unit_cost * p_quantity;

    -- Check if user has enough gold
    IF (SELECT gold FROM public.user_stats WHERE id = v_user_id) < v_total_cost THEN
    ...
    -- Add/Update Weapon in Inventory
    INSERT INTO public.user_weapons (user_id, weapon_type, tier, quantity)
    VALUES (v_user_id, p_weapon_type, p_tier, p_quantity)
    ON CONFLICT (user_id, weapon_type, tier)
    DO UPDATE SET quantity = user_weapons.quantity + p_quantity;

    -- Recalculate Stats (Attack/Defense/Spy/Sentry)
