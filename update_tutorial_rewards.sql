-- Migration: Update Tutorial Rewards for Step 5

CREATE OR REPLACE FUNCTION public.advance_tutorial(expected_step integer)
 RETURNS json
 LANGUAGE plpgsql
 SECURITY DEFINER
AS $function$
...
BEGIN
    ...
    ELSIF v_current_step = 4 THEN -- Economy Part 1 (Build Mine)
        v_gold_gain := 600;
        v_citizen_gain := 10;
        v_rewards := '{"Gold": 600, "Citizens": 10}';
    ELSIF v_current_step = 5 THEN -- Economy Part 2 (Hire Miners)
        v_xp_gain := 1000; -- UPDATED REWARD
        v_turn_gain := 25;
        v_rewards := '{"XP": 1000, "Turns": 25}';
