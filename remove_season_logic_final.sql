-- remove_season_logic_final.sql
-- Completely removes all season-related tables, functions, triggers, and logic.
...
DROP FUNCTION IF EXISTS get_season_info() CASCADE;
DROP FUNCTION IF EXISTS create_season_v2(integer, timestamp with time zone, timestamp with time zone) CASCADE;
DROP FUNCTION IF EXISTS check_and_start_scheduled_season() CASCADE;

-- 2. Drop the seasons table
DROP TABLE IF EXISTS seasons CASCADE;
DROP TABLE IF EXISTS season_winners CASCADE;

-- 3. Revert process_game_tick to remove season check (if it existed)
-- This function is generally complex, so we will replace it with a version that DEFINITELY has no season gating.
...
BEGIN
    ...
    SET
        ...
        -- Vault: If not over capacity, add interest
        vault = CASE
