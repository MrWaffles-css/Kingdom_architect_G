-- REBUILD Technology Research System (Attack, Defense, Spy, Sentry)
-- Run this in Supabase SQL Editor. This will reset the logic for tech upgrades.

-- 1. CLEANUP: Drop existing functions to ensure no conflicts
DROP FUNCTION IF EXISTS public.upgrade_research_attack();
DROP FUNCTION IF EXISTS public.upgrade_research_defense();
DROP FUNCTION IF EXISTS public.upgrade_research_spy();
DROP FUNCTION IF EXISTS public.upgrade_research_sentry();
DROP FUNCTION IF EXISTS public.get_tech_multiplier(int);
DROP FUNCTION IF EXISTS public.get_tech_video_cost(int);

-- 2. SCHEMA: Ensure columns exist in user_stats
...
-- 3. LOGIC: Helper - Get Tech Multiplier (Level 0-63)
...
-- 4. LOGIC: Helper - Get Tech Cost (XP)
...
-- 5. LOGIC: Main Stats Calculation (Updated with Multipliers)
...
-- 6. RPC: Upgrade Functions (With Role Permissions)
...
-- 7. REFRESH: Recalculate stats for all users and reload schema cache
DO $$
DECLARE
    v_user RECORD;
BEGIN
