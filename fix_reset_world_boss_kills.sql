-- Fix reset_world to include user_boss_kills deletion
-- This ensures the "defeated boss counter" is properly reset during world resets
...
BEGIN
    ...
    -- Delete all boss kill counts (FIXED: This was missing!)
    ...
    -- Delete all alliance messages (chat)
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'alliance_messages') THEN
        DELETE FROM alliance_messages WHERE TRUE;
    END IF;

    -- Delete all shared spy reports
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'shared_spy_reports') THEN
        DELETE FROM shared_spy_reports WHERE TRUE;
    END IF;
    ...
    RAISE NOTICE 'World has been reset successfully! All stats, items, research, boss progress, boss kill counts, alliance chats, and shared reports cleared.';
END;
$$;
