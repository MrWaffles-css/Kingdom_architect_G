-- Optimize Battle Logic - Update to return Report ID
-- Consolidates achievement tracking into the main attack_player function
-- Reduces network roundtrips from 4 to 1 per attack
-- Returns the ID of the generated report so the frontend can redirect to it
...
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public -- Ensure search_path is set to public for security
AS $$
DECLARE
    v_attacker_id uuid;
    ...
    v_kill_rate float;
    v_loss_rate float;
    v_report_id uuid; -- New variable to store the report ID
BEGIN
