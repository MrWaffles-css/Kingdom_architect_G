-- Refactor Hostage System to use per-level configuration
-- This gives deeper control as requested (Table format: Level | Cost | Bonus | Convert Cost)

-- 1. Modify the configuration table
ALTER TABLE public.hostage_configs 
ADD COLUMN IF NOT EXISTS levels jsonb DEFAULT '[]'::jsonb;

-- 2. Migration Function to populate existing data into new structure
CREATE OR REPLACE FUNCTION migrate_hostage_config() RETURNS void LANGUAGE plpgsql AS $$
DECLARE
    v_config record;
...
BEGIN
    ...
    -- If already migrated (levels is not empty), skip logic or overwrite?
    -- Let's overwrite to ensure consistency with current scalar values if they exist
    
    v_costs := v_config.research_costs;
    v_base_bonus := COALESCE(v_config.research_bonus_percent, 10);
    v_base_convert := COALESCE(v_config.convert_cost_gold, 2000);
    
    -- Limit to existing array length or default 10
    v_len := jsonb_array_length(v_costs);
    
    -- Level 0 (Base)
    -- Cost is 0 (you start here)
    v_new_levels := v_new_levels || jsonb_build_object(
        'level', 0,
        'cost', 0,
        'bonus', 0,
        'convert_cost', v_base_convert
    );
    
    -- Levels 1 to N
    FOR v_i IN 0..(v_len - 1) LOOP
        v_cost := (v_costs ->> v_i)::bigint;
        
        v_new_levels := v_new_levels || jsonb_build_object(
            'level', v_i + 1,
            'cost', v_cost,
            'bonus', (v_i + 1) * v_base_bonus,  -- e.g. Level 1 * 10 = 10%
            'convert_cost', v_base_convert     -- Flat across levels by default
        );
    END LOOP;
    
    UPDATE public.hostage_configs
    SET levels = v_new_levels
    WHERE id = 1;
END;
$$;

-- Run migration
SELECT migrate_hostage_config();

-- 3. Update 'get_hostage_config' to return the new structure (frontend will need to handle this)
-- We return everything for now
...
-- 4. Update 'update_hostage_config' to accept the levels array
CREATE OR REPLACE FUNCTION public.update_hostage_config_v2(
    p_levels jsonb
)
RETURNS json
LANGUAGE plpgsql
...
-- 5. Update Game Logic Functions to use 'levels' JSONB
...
DECLARE
    ...
    v_config record;
    v_levels jsonb;
    v_target_level_config jsonb;
    v_max_level int;
BEGIN
    -- Fetch Config
    SELECT * INTO v_config FROM public.hostage_configs WHERE id = 1;
    v_levels := v_config.levels;
    v_max_level := jsonb_array_length(v_levels) - 1; -- 0-based index means size 11 -> max lvl 10
    ...
    -- Get Config for NEXT level
    -- We assume the array is sorted by level, so index = level
    -- Ideally we search by key, but array index is faster and we control the structure.
    v_target_level_config := v_levels -> v_next_level;
    
    -- Safety check
    IF v_target_level_config IS NULL THEN
         RAISE EXCEPTION 'Configuration for level % not found', v_next_level;
    END IF;

    v_cost := (v_target_level_config ->> 'cost')::bigint;
    ...
    UPDATE user_stats 
    SET research_hostage_convert = v_next_level 
    WHERE id = v_user_id;
...
-- B. CONVERT HOSTAGES (Use convert_cost from current level)
...
BEGIN
    ...
    -- Handle case where level exceeds config (fallback to max?)
    -- Here we just take array item at index.
    v_level_config := v_config.levels -> v_current_level;
    
    IF v_level_config IS NULL THEN
        -- Fallback to last available if something is weird
        v_level_config := v_config.levels -> (jsonb_array_length(v_config.levels) - 1);
    END IF;

    v_cost_per_unit := (v_level_config ->> 'convert_cost')::int;
...
-- C. ATTACK PLAYER (Use bonus from current level)
...
BEGIN
    ...
    IF v_attacker_stats.attack > v_defender_stats.defense THEN
        ...
        SELECT * INTO v_config FROM public.hostage_configs WHERE id = 1;
        v_current_level := COALESCE(v_attacker_stats.research_hostage_convert, 0);
        ...
        v_bonus_percent := (v_level_config ->> 'bonus')::int;
        
        -- Rate: Explicit % from config
        v_hostage_rate := v_bonus_percent::float / 100.0;
